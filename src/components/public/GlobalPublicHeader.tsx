...
export function GlobalPublicHeader({ onRegister }: GlobalPublicHeaderProps) {
  ...
  return (
     ...
     */
    <header className="fixed inset-x-0 top-0 z-50 w-full px-1 pt-2 pb-3 sm:px-2 lg:px-2 bg-[var(--color-background)]/95 backdrop-blur-3xl">
      <nav
        aria-label={t('public.navigation.home')}
        className="flex min-h-16 items-center justify-between gap-4 rounded-[22px] border border-white/55 bg-surface/40 px-4 shadow-lg sm:px-6"
      >
        {/* Brand / Logo */}
        <Link
          to="/"

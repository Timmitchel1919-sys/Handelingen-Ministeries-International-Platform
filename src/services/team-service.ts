...
const repo = new FirestoreRepository<Omit<Team, 'id'>>('teams');

export const teamService = {
  async getById(churchId: string, id: string) {
    const team = await repo.getById(id);
    if (team.churchId !== churchId) {
      throw { kind: 'not-found', message: 'Team not found.' };
    }
    return team;
  },
  ...
  async create(data: Omit<Team, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt' | 'deletedBy' | 'archivedAt'>, actorUid: string) {
    return repo.create({
      ...data,
  ...
  async update(churchId: string, id: string, data: Partial<Omit<Team, 'id' | 'createdAt' | 'createdBy' | 'churchId'>>, actorUid: string) {
